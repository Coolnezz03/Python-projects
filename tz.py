...
def process_product(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'Номер кузова' in product_data.keys():
                product_data['Кузов'] = product_data['Номер кузова']
                del product_data['Номер кузова']
            if 'Номер двигателя' in product_data.keys():
                product_data['Двигатель'] = product_data['Номер двигателя']
                del product_data['Номер двигателя']
            if 'Тип шасси' in product_data.keys():
                product_data['Комментарий'] = product_data['Тип шасси']
                del product_data['Тип шасси']
            if 'Марка' in product_data.keys():
                product_data['Производитель'],product_data['Модель'] = product_data['Марка'].split(' ', maxsplit=1)
                del product_data['Марка']
            else:
                product_data['Производитель'], product_data['Модель'] = None, None
...
def main():
    ...
    df['Примечание'] = df['Примечание'].str.replace(r'(Прим:|Цвет:)', '', regex=True).str.strip()
    df.to_excel(f"tz{random.randint(1,1100)}.xlsx", index=False)
